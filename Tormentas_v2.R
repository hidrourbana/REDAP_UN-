...
for (i in (2:(vindex-t))){
  if ((index[i]>(index[i-1]+e))&(index[i]==index[i+t]-t)){
    c2 = c2+1
    a=a+1
    matches[a]<-c(i) 
  }
...
tormenta_final<-prueba[c(matches_index),]
...
for (i in 1:vtormenta) {
  at<-at+1
  tormenta[matches[i]:matches2[i]]<-prueba[matches_index[i]:matches2_index[i],1]
}
...
df_tormentas<-data.frame("No.Tormenta"=(1:length(fechas)),"Fecha de Inicio"=fechas)
